    ...
    if ruta_base(year_app).exists():
        try:
            ...
            with st.expander("Seleccionar semanas de vacaciones fijas", expanded=True):
                ...
                # Mantiene la última versión válida de la tabla.
                # Esto recupera el comportamiento anterior de max_selections:
                # no deja conservar más cuadritos marcados que las vacaciones disponibles.
                clave_vacaciones_validas = f"vacaciones_fijas_validas_{year_app}"
                clave_version_vacaciones = f"vacaciones_fijas_version_{year_app}"
                clave_mensaje_vacaciones = f"vacaciones_fijas_mensaje_{year_app}"

                if clave_version_vacaciones not in st.session_state:
                    st.session_state[clave_version_vacaciones] = 0
                ...
                df_vacaciones_editor = st.data_editor(
                    ...
                    num_rows="fixed",
                    key=f"tabla_vacaciones_fijas_{year_app}_{st.session_state[clave_version_vacaciones]}",
                )
                ...
                if errores_temporales_vacaciones:
                    st.session_state[clave_mensaje_vacaciones] = (
                        "No se aplicó la última selección porque excedía las semanas de vacaciones disponibles.\n\n"
                        + "\n".join([f"- {e}" for e in errores_temporales_vacaciones])
                    )
                    st.session_state[clave_version_vacaciones] += 1
                    if hasattr(st, "rerun"):
                        st.rerun()
                    else:
            ...
            if ruta_anual(year_app).exists():
                ...
                t1, t2 = st.tabs(["📊 Calendario anual", "📈 Resumen"])
                with t1:
                    mostrar_dataframe_estilado(df_anual_guardado)
                with t2:
